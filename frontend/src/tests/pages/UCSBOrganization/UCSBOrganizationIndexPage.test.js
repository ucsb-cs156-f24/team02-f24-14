...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders three organizations correctly for regular user", async () => {
    ...
    const createOrganizationButton = screen.queryByText("Create");
    expect(createOrganizationButton).not.toBeInTheDocument();

    //const orgCode = screen.getByText("BAL");
    //expect(orgCode).toBeInTheDocument();

    const orgTranslationShort = screen.getByText("Basketball Club");
    expect(orgTranslationShort).toBeInTheDocument();

    const orgTranslation = screen.getByText("UCSB Basketball Club");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "Organization with orgCode BAL was deleted");
    ...
    await waitFor(() => {
      expect(mockToast).toBeCalledWith(
        "Organization with orgCode BAL was deleted",
      );
    });
    ...
    expect(axiosMock.history.delete[0].url).toBe("/api/ucsborganization");
    expect(axiosMock.history.delete[0].params).toEqual({ orgCode: "BAL" });
  });
});